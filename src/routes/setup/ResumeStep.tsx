...
export default function ResumeStep() {
  ...
  useEffect(() => {
    if (!sessionId || reparsing || !reached(state, "resume.ready") || profile)
      return;
    let active = true;
  ...
  useEffect(() => {
    if (state.stage === "resume" && state.status === "failed")
  ...
  useEffect(() => {
    if (reparsing && state.stage === "resume" && state.status !== "ready")
      setReparsing(false);
  ...
  const onPreviewValidity = useCallback(
    (valid: boolean) => setPreviewValid(valid),
    [],
  );
            ...
            {pendingResume.type === "application/pdf" ? (
              ...
              >
                <PdfPreview
                  file={pendingResume}
                  maxPages={3}
                  onValidityChange={onPreviewValidity}
                />
              </Suspense>
            ) : (
      ...
      <Modal open={showResumeError} onClose={() => setShowResumeError(false)}>
        <div className="text-center">
          ...
          <p className="mt-1.5 text-sm text-slate-600">
            Something went wrong while parsing your resume. Please try uploading
            it again — a PDF or DOCX export usually works best.
          </p>
