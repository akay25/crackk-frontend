...
export default function ReportView({ report, role }: { report: ReportData; role: string | null }) {
    ...
    <div className="space-y-8">
      ...
      <section className="rounded-3xl bg-gradient-to-r from-indigo-600 via-indigo-600 to-violet-600 px-6 sm:px-8 py-8 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
          <div>
            ...
            <h3 className="mt-1 text-2xl sm:text-3xl font-bold text-white tracking-tight">{REPORT_PLACEHOLDER.onsiteVerdict}</h3>
          </div>
          <div className="sm:text-right">
            <p className="text-indigo-200 text-xs font-semibold uppercase tracking-wide">Confidence</p>
            <p className="mt-1 text-3xl font-bold text-white tabular-nums">{REPORT_PLACEHOLDER.confidencePct}%</p>
            <div className="mt-2 h-1.5 w-40 rounded-full bg-indigo-400/40 overflow-hidden sm:ml-auto">
              <div className="h-full rounded-full bg-white" style={{ width: `${REPORT_PLACEHOLDER.confidencePct}%` }} />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white rounded-3xl border border-slate-200/80 shadow-sm px-6 sm:px-8 py-8">
        ...
        <ul className="mt-6 space-y-4">
          {REPORT_PLACEHOLDER.recommendations.map((r) => (
